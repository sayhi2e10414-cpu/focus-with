...
- **Earned rewards stay earned.** Choose or create a reward, build an uninterrupted focus streak toward it, and redeem it later from a persistent reward cabinet.
...
- Editable uninterrupted-focus rewards with a default example, selectable targets, persistent earned inventory, and automatic camera/blocklist/timer evidence fallback.
- Built-in companion using OpenAI Responses, Anthropic, or an OpenAI-compatible API, including compatible local endpoints.
- Eleven annotated MCP tools for Claude Desktop/Code, Codex, and other local clients, including reward progress and redemption.
...
- Uninterrupted-focus rewards: [docs/REWARDS.md](docs/REWARDS.md)
- iPhone Shortcuts: [docs/IPHONE_SHORTCUTS.md](docs/IPHONE_SHORTCUTS.md)
- Android companion: [docs/ANDROID.md](docs/ANDROID.md)
- Signed Android and experimental Windows downloads: [GitHub Releases](https://github.com/sayhi2e10414-cpu/focus-with/releases)
...
- Camera frames stay inside the companion app. Distraction events contain only an ID, sustained duration, timestamp, and source; reward heartbeats contain only timestamp, source, and camera state. Image-derived fields are rejected.
